     ...
     filtre = filtre.toLowerCase()
      ...
      if (filters.includes(filtre.toLowerCase())) {
        if (queue.filters.has(filtre)) {
          ...
          embed.setDescription(lang.msg31.replace("{filter}", filtre).replace("{status}", "<:cross:1023313041854570558>"))
          return interaction.editReply({ embeds: [embed] }).catch(e => { })
        } else {
          queue.filters.add(filtre)
          embed.setDescription(lang.msg31.replace("{filter}", filtre).replace("{status}", "<:valid:1021966974877696050>"))
          return interaction.editReply({ embeds: [embed] }).catch(e => { })
        }
      } else {
